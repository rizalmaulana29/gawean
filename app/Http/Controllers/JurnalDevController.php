...
class JurnalDevController extends Controller
{
    ...
    public function ApllyCreditMemo2 ($getDataTransaksi,$transaction_no,$sisaBayar){
      ...
      if ($err) {
          ...
          if ($searchResponse == true){
              $dataResponse = json_decode($response);
              $updatePayment = Payment::where('id_transaksi',$getDataTransaksi['id_transaksi'])->update(['receive_payment' => $dataResponse->customer_apply_credit_memo->id]);
              $response = array("status" => true,
                              "message"=> $dataResponse->customer_apply_credit_memo);
          }
          else{
              $updatePayment= Payment::where('id_transaksi',$getDataTransaksi['id_transaksi'])->update(['receive_payment' => "failed"]);
              $response = array("status"=>false,"message"=> "apply credit memo 2".$response);
          }
      }
